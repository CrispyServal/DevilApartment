...
impl ChunkUpdater {
    ...
    pub fn simulate(&self, world_buffer: &WorldBuffer) {
        ...
        if active_range.max_x > active_range.min_x && active_range.max_y > active_range.min_y {
            println!("chunk: {}, {} active range: {}, {}, {}, {}", self.chunk_x, self.chunk_y, active_range.min_x, active_range.max_x, active_range.min_y, active_range.max_y)
        }
        for world_y in
            (self.start_world_y + active_range.min_y..self.start_world_y + active_range.max_y).rev()
        {
                ...
                self.start_world_x + active_range.min_x..self.start_world_x + active_range.max_x
            {
                ...
                if let Some((final_x, final_y)) =
                    pixel.try_move_self(world_buffer, world_x, world_y)
                {
                    if final_x != world_x || final_y != world_y {
                        let tmp = world_buffer.get_pixel(final_x, final_y);
                        world_buffer.set_pixel(final_x, final_y, pixel);
